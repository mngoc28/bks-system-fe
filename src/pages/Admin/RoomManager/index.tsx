...
const RoomManager: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    const savedMode = localStorage.getItem("roomManager_viewMode");
    return (savedMode as ViewMode) || "table"; // Default to table
  });

  const handleViewModeChange = (mode: ViewMode) => {
    setViewMode(mode);
    localStorage.setItem("roomManager_viewMode", mode);
  };
          ...
          {viewMode === "grid" ? (
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <RoomTable
                    ...
                    sort={sortField ? { key: sortField as any, direction: sortDirection || "asc" } : null}
                    filters={filters}
