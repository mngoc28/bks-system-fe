...
const QuestionManagerPage = () => {
  ...
  const { filters, searchValue, setTitle, setPage, setPerPage, reset, toggleSort } = useQuestionFilters();

  // View mode state with localStorage persistence
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    const savedMode = localStorage.getItem("questionManager_viewMode");
    return (savedMode as ViewMode) || "table"; // Default to table
  });

  const handleViewModeChange = (mode: ViewMode) => {
    setViewMode(mode);
    localStorage.setItem("questionManager_viewMode", mode);
  };
  ...
  return (
    <AdminPageShell>
      <PageBar
        subtitle={t("questions.subtitle") || "Qu?n lý lu?ng câu h?i và k?ch b?n chatbot t? d?ng."}
