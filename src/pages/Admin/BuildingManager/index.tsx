...
import { BuildingHeader, BuildingSearchSection, BuildingCard, DeleteConfirmDialog, BuildingTable } from "./components";
...
const Buildings: React.FC = () => {
  ...
  // Sort logic
  const { sort, clearSort, toggleSort, getSortDirection } = useBuildingSort();

  // View mode state with localStorage persistence
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    const savedMode = localStorage.getItem("buildingManager_viewMode");
    return (savedMode as ViewMode) || "table"; // Default to table
  });

  const handleViewModeChange = (mode: ViewMode) => {
    setViewMode(mode);
    localStorage.setItem("buildingManager_viewMode", mode);
  };
