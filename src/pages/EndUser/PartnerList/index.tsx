        ...
        <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-sky-50/40 text-slate-900">
                ...
                <div className="mx-auto max-w-[1440px] py-2.5 px-4 sm:px-6 lg:px-8">
                    <Breadcrumb
                        items={[
                            ...
                            { label: t("public.newsList.breadcrumb"), href: ROUTERS.PUBLIC_NEWS_LIST },
                            { label: provinceName }
                        ]}
                        className="text-sm"
                    />
            ...
            <div className="mx-auto w-full max-w-[1440px] px-4 pt-8 sm:px-6 lg:px-8 space-y-2">
                <span className="text-[10px] uppercase tracking-[0.25em] font-extrabold text-sky-600 block">
                    {t("endUserPartners.hero_badge")}
                </span>
                <h1 className="text-3xl font-black text-slate-900 tracking-tight">
                    {t("endUserPartners.hero_title")}{" "}
                    <span className="text-sky-600">{provinceName}</span>
                </h1>
                <p className="text-sm text-slate-500 max-w-4xl leading-relaxed">
                    {t("endUserPartners.hero_sub")}
                </p>

                {/* Stat cards */}
                <div className="flex flex-wrap gap-3 pt-2">
                    <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50/50 px-4 py-2 shadow-sm">
                        ...
                        <div>
                            ...
                            <div className="text-[9px] uppercase font-bold text-slate-400 tracking-wider">Đối tác</div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50/50 px-4 py-2 shadow-sm">
                        <MapPin className="size-4 text-emerald-600" />
                        <div>
                            <div className="text-sm font-bold text-slate-950">{provinceName}</div>
                            <div className="text-[9px] uppercase font-bold text-slate-400 tracking-wider">Khu vực</div>
                        </div>
                    </div>
                </div>
            </div>

            <main className="mx-auto max-w-[1440px] space-y-16 px-4 py-12 sm:px-6 lg:px-8">
                ...
                <section className="space-y-8">
                    ...
                    {isLoading ? (
                        ...
                        <div className="col-span-full rounded-[40px] border-2 border-dashed border-slate-200 bg-slate-50 py-24 text-center">
                            ...
                            <Button asChild variant="outline" className="rounded-full border-slate-200 px-8 hover:border-primary/40 hover:bg-primary-light hover:font-bold hover:text-primary">
                              <Link to={ROUTERS.PUBLIC_NEWS_LIST}>{t("endUserPartners.back_to_news")}</Link>
                            </Button>
                        </div>
                    ) : (
                          ...
                          >
                            ...
                            <div className="flex min-w-0 flex-1 flex-col gap-1">
                              ...
                              <div className="flex items-center gap-1.5 text-sm text-slate-500 mt-0.5">
                                ...
                                <span className="truncate">{partner.address || t("public.roomByProvince.fallbackAddress")}</span>
                              </div>
                            </div>
                  ...
                  {hasPartners && (
                    <div className="text-center">
                      ...
                      >
                        {t("endUserPartners.view_all_link")}
                      </Button>
                    </div>
                  )}
                </section>
                ...
                <section className="flex flex-col items-center gap-8 rounded-[40px] border border-primary/10 bg-primary-light px-8 py-10 shadow-sm md:flex-row md:px-12">
                  ...
                  <div className="flex-1 space-y-2 text-center md:text-left">
                    <h3 className="text-xl font-black text-slate-900">
                      {t("endUserPartners.info_banner_title")}
                    </h3>
                    <p className="font-medium leading-relaxed text-slate-600">
                      {t("endUserPartners.info_banner_sub")}
                    </p>
                  </div>
                  <Button asChild className="h-14 rounded-full bg-primary px-8 font-black shadow-lg shadow-primary/20 hover:bg-primary-hover">
                    <Link to={`${ROUTERS.PUBLIC_FAQ}?category=rewards`}>
                      {t("endUserPartners.info_banner_cta")}
                    </Link>
                  </Button>
                </section>
                ...
                <section className="space-y-6">
                  <h2 className="text-2xl font-black text-slate-900">
                    {t("endUserPartners.accordion_title", { province: provinceName })}
                  </h2>
                      ...
                      <div key={item.id} className="overflow-hidden rounded-2xl border border-slate-100 transition-colors hover:border-primary/20">
                        ...
                        {openAccordion === item.id && (
                          <div className="border-t border-primary/10 bg-white px-8 py-6 text-sm font-medium leading-relaxed text-slate-600 duration-300 animate-in fade-in slide-in-from-top-2">
                            {t("endUserPartners.accordion_content")}
                          </div>
                        )}
                      </div>
                    ))}
                ...
                <section className="space-y-12">
                  <h2 className="text-center text-2xl font-black text-slate-900">
                    {t("endUserPartners.features_title")}
                  </h2>
                  <div className="grid grid-cols-1 gap-10 md:grid-cols-3">
                    {[
                      { 
                        icon: <ShieldCheck className="size-8 text-primary" />, 
                        title: t("endUserPartners.features.item1Title"),
                        desc: t("endUserPartners.features.item1Desc"),
                      },
                      { 
                        icon: <Building2 className="size-8 text-primary" />, 
                        title: t("endUserPartners.features.item2Title"),
                        desc: t("endUserPartners.features.item2Desc"),
                      },
                      { 
                        icon: <CreditCard className="size-8 text-primary" />, 
                        title: t("endUserPartners.features.item3Title"),
                        desc: t("endUserPartners.features.item3Desc"),
                      }
                    ].map((feature, idx) => (
                ...
                <section className="relative overflow-hidden rounded-[40px] bg-slate-900 text-white shadow-2xl">
                  ...
                  <div className="relative flex flex-col items-center gap-12 px-8 py-16 md:flex-row md:px-16">
                    ...
                    <div className="shrink-0 space-y-4">
                      ...
                      <p className="text-center text-xs font-bold uppercase tracking-widest text-slate-400">{t("common.response_time")}</p>
                    </div>
                  </div>
                </section>
